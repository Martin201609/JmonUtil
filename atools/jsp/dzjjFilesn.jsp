...
<%
    ...
    if (button == null) {
        message = "手动维护作业";
        text.append("");
    } else if (button.equals("MAKEDIR") && (path.startsWith("/") || path.endsWith("/"))) {
        if (file.exists() == false) {
            file.mkdirs();
            message = "folder maked:" + file.getAbsolutePath();
        } else {
            message = "folder existed:" + file.getAbsolutePath();
        }
    ...
    <table border="0" width="100%" cellpadding="0" cellspacing="2">
        <tr class=deep-bg-left>
            ...
            <td colspan=3><%= rootPath.replace(File.separatorChar, '/') %>
                <input type=text name=FilePath size=30 value="<%= path %>">
        ...
        <tr>
            <td colspan=2>
                <input type=submit name=Button value="MAKEDIR">
